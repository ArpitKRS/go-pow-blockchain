...
func main() {
	...
	go func() {
		t := time.Now()
		genesisBlock := Block{}
		genesisBlock = Block{0, t.String(), 0, calcHash(genesisBlock), "", difficulty, ""}
...
func generateBlock(oldBlock Block, Data int) Block {
	...
	for i := 0; ; i++ {
		hex := fmt.Sprintf("%x", i)
		newBlock.Nonce = hex
		if !isHashValid(calcHash(newBlock), newBlock.Difficulty) {
			fmt.Println(calcHash(newBlock), "Keep Calculating...")
			time.Sleep(time.Second)
			continue
		} else {
			fmt.Println(calcHash(newBlock), "Done!!")
